...
// Foto de portada por propiedad desde Supabase Storage: la tabla
// propiedad_image_sets liga cada propiedad (uuid) con una carpeta del bucket
// público Imagenes ({image_set_id}/1.jpg, 2.jpg, ...). Se toma la imagen
// llamada portada/fachada/cover si existe, o la primera en orden natural,
// vía el endpoint de transformación (los originales pesan >10MB).
export async function getInventarioImagesByIds(
  ...
  const urlBySet = new Map<number, string>()
  await Promise.all(
    [...new Set(setByProp.values())].map(async (setId) => {
      const { data: files } = await supabase.storage
        .from(IMAGES_BUCKET)
        .list(String(setId), { limit: 100 })
      const images = (files ?? []).filter((f) => IMAGE_FILE_RE.test(f.name))
      if (images.length === 0) return
      const portada = images.find((f) => /portada|fachada|cover/i.test(f.name))
      const first =
        portada ??
        images.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))[0]
      const { data: pub } = supabase.storage
        .from(IMAGES_BUCKET)
        .getPublicUrl(`${setId}/${first.name}`)
      urlBySet.set(
        setId,
        pub.publicUrl.replace('/object/public/', '/render/image/public/') + '?width=1600&quality=80'
      )
    })
  )
  ...
  for (const [propId, setId] of setByProp) {
    const url = urlBySet.get(setId)
    if (url) map[propId] = url
  }
  return map
}
